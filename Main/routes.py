...
from Main.scraper import get_url_to_openlib, add_genre_to_database, scrape_actual_info
import pymysql
import json
...
def addBooks():
    ...
    bauth = request.form['bauth']
    print(bname, bauth)
    url, name = get_url_to_openlib(bname, bauth)
    res = scrape_actual_info(url)
    add_genre_to_database(name, bid, res, conn=con, author=bauth)
    bid+=1
...
def returnb():
    ...
    end_date =((datetime_obj + datetime.timedelta(days=7)).strftime(format_str))
    print(end_date)
    ...
    query2="update books set isAvailable=1 where bid={}".format(request.form['rev'][0])
    result2 = execute_query(query2, con)
    con.commit()
    print(query2)
    print(result2)
    query3 = "update issued set is_returned=1 where bid={}".format(request.form['rev'][0])
    c3 = con.cursor()
    c3.execute(query=query3)
    result3 = c3.fetchall()
    con.commit()
